...
- Empty cell = 0x2800
- `pixelMap` order is meaningful and should not be alphabetized
...
5. Optional: add a tiny legend for the “all 8 dots” sample
...
1. **`go.mod`** - Initialize module with `github.com/cboone/stipple`
2. **Update `README.md`** - Change import paths from `github.com/stipple/stipple` to `github.com/cboone/stipple`
   - Update any other docs or examples that reference the old import path
...
- Negative fractional values floor to -1 and are treated as out-of-bounds
...
- If width or height is not divisible by 2 or 4 respectively, define truncation behavior for `Rows()` and `Cols()` and cover it in tests
...
| File | Action | Purpose |
|------|--------|---------|
| `go.mod` | Create | Module definition (`github.com/cboone/stipple`) |
| `README.md` | Update | Fix import paths to `github.com/cboone/stipple` |

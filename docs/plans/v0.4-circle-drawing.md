...
Add circle drawing to stipple using Bresenham's midpoint circle algorithm.
...
- Use `x <= y` to include diagonal pixels for small radii
...
| TestCircleRadius1 | Small outline circle has 4 cardinal points |
...
- Avoid exact pixel counts for outline circles; assert symmetry and key points
- For filled circles, validate each scanline between first and last set pixel is contiguous
- Keep tests deterministic with small radii that have obvious expected points
- Add a diagonal point assertion for `radius = 1` to ensure `x == y` handling is exercised
- Add visual snapshot expectations using golden files for a few small circles
  - Store ASCII frames in `draw/testdata/`
  - Compare `c.Frame()` to the golden file contents
  - Provide a flag to update snapshots when changes are intentional
...
3. Preserve existing ordering conventions (imports, demo numbering, spacing)
